...
const quickPrompts = [
  { icon: "😟", text: "I'm feeling anxious today" },
  { icon: "😴", text: "I can't sleep well lately" },
  { icon: "🧘", text: "Guide me through a breathing exercise" },
  { icon: "📝", text: "Give me a journaling prompt" },
];
...
export default function Chat() {
  ...
  const streamChat = async (allMessages: Message[]) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "I'm sorry, I couldn't respond right now. Please try again. 🌿" },
      ]);
    } finally {
      setIsLoading(false);
    }
        ...
        <Card className="flex-1 flex flex-col overflow-hidden border-border/30 bg-card/50 backdrop-blur-sm">
          <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[300px]">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-6 py-8">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground max-w-sm">
                    I'm your wellness companion. Tell me how you're feeling, or try one of these:
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
                  {quickPrompts.map((p) => (
                    <button
                      ...
                      className="text-left p-3 rounded-xl border border-border/50 bg-background/50 hover:bg-primary/5 hover:border-primary/30 transition-all text-sm group"
                    >
              ...
              messages.map((msg, i) => (
                ...
                >
                  {msg.role === "assistant" && (
                    <div className="gradient-accent p-1.5 rounded-lg h-fit mt-1 shrink-0">
                  ...
                  {msg.role === "user" && (
                    <div className="bg-secondary p-1.5 rounded-lg h-fit mt-1 shrink-0">
            ...
            {isLoading && messages[messages.length - 1]?.role !== "assistant" && (
              <div className="flex gap-3 items-start animate-fade-up">
                <div className="gradient-accent p-1.5 rounded-lg shrink-0">
                  <Bot className="h-4 w-4 text-primary-foreground" />
                </div>
          ...
          <div className="border-t border-border/30 p-3">
            ...
            <p className="text-[10px] text-muted-foreground text-center mt-2 flex items-center justify-center gap-1">
              <Heart className="h-3 w-3" /> Not a substitute for professional mental health care
            </p>
          </div>
        </Card>
      </main>
