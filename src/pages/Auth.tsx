...
const displayNameSchema = z.string().trim().min(1, { message: "Display name is required" }).max(50, { message: "Display name must be less than 50 characters" });
...
export default function Auth() {
  ...
  const validateForm = (isSignUp: boolean) => {
    ...
    if (isSignUp) { const r = displayNameSchema.safeParse(displayName); if (!r.success) newErrors.displayName = r.error.errors[0].message; }
    setErrors(newErrors);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(false);
    if (error) { toast.error(error.message.includes("already registered") ? t("auth.alreadyRegistered") : error.message); }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <div className="flex items-center gap-2 mb-8 animate-fade-up">
        <div className="p-2 rounded-xl bg-primary/10"><Heart className="h-8 w-8 text-primary" /></div>
        <span className="font-display text-2xl font-bold text-foreground">MindfulMe</span>
      </div>
      
      <Card className="w-full max-w-md gradient-card border-border/30 shadow-elevated animate-fade-up" style={{ animationDelay: "100ms" }}>
              ...
              <form onSubmit={handleSignIn} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="signin-email" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="pl-10" />
                  </div>
              ...
              <form onSubmit={handleSignUp} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="signup-name" type="text" placeholder={t("auth.displayNamePlaceholder")} value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="pl-10" />
                  </div>
      ...
      <p className="mt-6 text-sm text-muted-foreground text-center animate-fade-up" style={{ animationDelay: "200ms" }}>
        {t("auth.journeyStart")}
      </p>
    </div>
  );
